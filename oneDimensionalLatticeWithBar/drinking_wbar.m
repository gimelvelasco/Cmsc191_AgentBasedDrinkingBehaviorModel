...
for gencnt = 1:Generations
    switch(gencnt) %1 bar is established at generation 2
        case 2
            Bar = randi([1,N]);
            B(Bar) = B(Bar) + 1;
    end
    ...
    for mcnt = 1:N
        ...
        for lcnt = 1:Dcell
            if gencnt < 2
                ...
                if abs(mcnt-Bar) <= 10 %if current drinker is within proximity of a bar, it is more likely to move closer towards the bar.
                    switch(mcnt)
                        case 1 %left most site can only move right
                            if (mcnt-Bar) == 0 %if drinker is at the bar, it is less likely to move
                                if rand() > pb
                                    D(mcnt) = D(mcnt) - 1;
                                    D(mcnt+1) = D(mcnt+1) + 1;
                                end
